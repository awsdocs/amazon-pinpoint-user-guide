# Using advanced template capabilities<a name="message-template-helpers"></a>
...
+ [Default](#defaulthelpers) 
+ [Conditional](#conditionhelpers) 
+ [String](#stringhelpers)
+ [Math and encoding](#customhelpers)
+ [Inline partials](#inlinepartials)
...
This section describes the **built\-in** helpers provided by Handlebars\. For the full list see [Built\-in Helpers](https://handlebarsjs.com/guide/builtin-helpers.html) at [handlebarsjs\.com](https://handlebarsjs.com)\.
...
+ `{{#neg valuea valueb}}`
  ...
  `neg` must be prefaced with a pound sign \(`#`\) and conclude with a closing `{{/neg}}` in the block statement\.
**Example**  
...
`{{#neg User.UserAttributes.Favorite.Colors[0] "red"}}`  
*You do not like red\.*  
`{{else}}`  
*You like red\.*  
`{{/neg}}`
...
This section describes the **string **helpers\.
...
**Usage**  
`{{dateFormat date ["format"] [format="f"][tz=timeZoneId]}}`  
...
**Example**  
In this example, a message is sent to a user using the `full` date format based on the *America/Los\_Angeles* time zone\.  
`We can meet with you any time on ``{{dateFormat date format="full" tz=America/Los_Angles}}.` returns  
...
## Math and encoding helpers<a name="customhelpers"></a>
...
While technically not a helper, inline partials are Handlebars way to simplify templates that include repeated strings, allowing for easier reuse\. For more details, see For more information see [Inline partials](https://handlebarsjs.com/guide/partials.html#inline-partials) at [handlebarsjs\.com](https://handlebarsjs.com)\. 
